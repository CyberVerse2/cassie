...
describe("Polymarket env", () => {
  it("reads and normalizes CLOB execution config from env", () => {
    const config = readPolymarketExecutionEnv({
      ...
      POLYMARKET_RELAYER_API_KEY_ADDRESS:
        "0x193c2109089dD260811f1852C9B1521D6CCF1c6B",
    });
  ...
  it("reads and validates Hyperliquid execution config centrally", () => {
    ...
    expect(assertHyperliquidExecutionEnv(config).privateKey).toBe(
      `0x${"2".repeat(64)}`,
    );
    expect(() =>
      assertHyperliquidExecutionEnv(readHyperliquidExecutionEnv({})),
    ).toThrow(MissingConnectorConfigError);
  });

  it("rejects invalid private keys and signature types", () => {
    expect(() => normalizePrivateKey("not-a-key")).toThrow(
      "POLYMARKET_PRIVATE_KEY must be a 32-byte hex private key.",
    );
    expect(() =>
      readPolymarketExecutionEnv({
  ...
  it("centralizes generic required connector env validation", () => {
    expect(() =>
      requiredConnectorEnv("DeepSeek", "DEEPSEEK_API_KEY", {}),
    ).toThrow(MissingConnectorConfigError);
    expect(
      requiredConnectorEnv("DeepSeek", "DEEPSEEK_API_KEY", {
  ...
  it("builds an encoded Postgres URL from raw container database parts", () => {
    expect(
      readDatabaseUrl({
        DATABASE_URL:
          "postgres://postgres:wrong@remote.example.com:5432/cassie",
  ...
  it("validates Privy treasury settlement config", () => {
    expect(() =>
      ...
      "PRIVY_AUTHORIZATION_PRIVATE_KEY, CASSIE_TREASURY_WALLET_ID, CASSIE_TREASURY_WALLET_ADDRESS",
    );
    ...
    expect(
      assertPrivySettlementEnv(
        readCassieConfig({
          ...
          CASSIE_TREASURY_WALLET_ADDRESS:
  ...
  it("centralizes numeric env defaults", () => {
    expect(
      numberEnv("COUNT", 3, { COUNT: "4.8" }, { integer: true, min: 1 }),
    ).toBe(4);
    expect(numberEnv("COUNT", 3, {})).toBe(3);
    expect(numberEnv("COUNT", 3, { COUNT: "" })).toBe(3);
    expect(() => numberEnv("COUNT", 3, { COUNT: "nope" })).toThrow(
      "COUNT must be a number.",
    );
    expect(() => numberEnv("COUNT", 3, { COUNT: "-1" }, { min: 1 })).toThrow(
      "COUNT must be at least 1.",
    );
    expect(() => numberEnv("COUNT", 3, { COUNT: "61" }, { max: 59 })).toThrow(
      "COUNT must be at most 59.",
    );
    expect(() =>
      readCassieConfig({ CASSIE_STRUCTURED_MAX_RETRIES: "0" }),
  ...
  it("builds one validated runtime config object", () => {
    ...
    const config = readCassieConfig({
      ...
      X_WEBHOOK_URL: "https://cassie.trade/api/x/webhook",
      X_ACCOUNT_ACTIVITY_WEBHOOK_ID: "456",
      X_WEBHOOK_ACCOUNT_ID: "2060718466630406149",
    ...
    expect(config).toMatchObject({
      ...
      x: {
        ...
        webhookAccountId: "2060718466630406149",
