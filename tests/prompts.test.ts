...
const opportunityFrame: OpportunityFrame = {
  ...
  marketImplication:
    "Potential bullish SUI read-through if activity increases.",
  userIntent: "trade",
...
const tradeExpression: TradeExpressionPlan = {
  ...
  coreInterpretation:
    "Lower transfer friction may increase Sui network activity.",
  directAsset: "SUI",
  ...
  highestPurityExpression:
    "Long SUI spot/perp if a real liquid venue validates the expression.",
  ...
  marketDiscovery: {
    ...
    instructions: "Search configured crypto venues for SUI.",
    queries: [],
  },
};
...
describe("prompts", () => {
  it("keeps semantic rules but removes AI SDK structured-output prose", () => {
    ...
    expect(prompts.join("\n").match(/Role:/g)?.length).toBeGreaterThanOrEqual(
      6,
    );
    expect(
      prompts.join("\n").match(/When uncertain:/g)?.length,
    ).toBeGreaterThanOrEqual(6);
    expect(
      prompts.join("\n").match(/Examples:/g)?.length,
    ).toBeGreaterThanOrEqual(6);
    expect(
      prompts.join("\n").match(/Before returning, verify internally:/g)?.length,
    ).toBeGreaterThanOrEqual(6);
    expect(prompts.join("\n")).toContain(
    ...
    expect(prompts.join("\n")).toContain(
      "Only configured venues can produce executable candidates",
    );
    expect(prompts.join("\n")).toContain("options and volatility instruments");
    expect(prompts.join("\n")).toContain(
      "Generate abstract candidateExpressions for venue discovery",
    );
    expect(prompts.join("\n")).toContain("Do not assume a real market exists");
    expect(prompts.join("\n")).toContain(
      "Do not use no_trade when a non-no_trade candidateExpression still needs venue discovery",
    );
    expect(prompts.join("\n")).toContain(
      "HIP-3 pre-stock/private-company valuation perps",
    );
    expect(prompts.join("\n")).toContain(
      "thesis resolves to yes or no by a specific date",
    );
    expect(prompts.join("\n")).toContain(
      "intendedSide must be the side that would resolve true if the source claim is true",
    );
    expect(prompts.join("\n")).toContain(
      "do not generate intendedSide no merely because the report might be overstated",
    );
    expect(prompts.join("\n")).toContain("additive to the price expression");
    expect(prompts.join("\n")).toContain("Cheap odds are not a fit defect");
    expect(prompts.join("\n")).toContain(
      "Fed will cut rates at the June FOMC meeting",
    );
    expect(prompts.join("\n")).toContain(
      "selectedMarket is one of the supplied candidates",
    );
    expect(prompts.join("\n")).toContain(
      "Prefer Hyperliquid perps over Hyperliquid spot",
    );
    expect(
      polymarketDiscoveryQueryPrompt({
        thesis,
      ...
      "Return an empty query list when the framed expression has no date-bounded yes/no event",
    );
    expect(
      polymarketSearchResultSelectionPrompt({
        thesis,
  ...
  it("exposes stage prompts as AI SDK-ready specs", () => {
    ...
    expect(
      sourceModeClassificationPromptSpec({
        sourcePost,
        userCommand: "@cassiedottrade urgent trade this",
      }).tools,
    ).toEqual({
      webSearch: {
  ...
  it("uses a slim rank-focused trade-expression payload for market selection", () => {
      ...
      candidateExpressions: [
        {
          ...
          thesis:
            "BTC short if the market reads through Strategy sale pressure.",
          whyThisExpressesTheOpportunity:
            "It expresses the BTC price read-through.",
    ...
    expect(payload.tradeExpression).not.toHaveProperty(
      "marketDiscoveryConfidence",
    );
    expect(payload.tradeExpression).not.toHaveProperty(
    ...
    expect(payload.tradeExpression.abstractExpressions[0]).not.toHaveProperty(
      "abstractMarket",
    );
    expect(payload.tradeExpression.abstractExpressions[0]).not.toHaveProperty(
      "relatedEntities",
    ...
    expect(prompt).not.toContain(
      "Verbose candidate text that should not be duplicated in ranking input.",
    );
    expect(prompt).not.toContain(
      "Abstract rank that should not be mistaken for final market selection.",
    );
  ...
  it("uses a slim fit-focused trade-expression payload for market assessment", () => {
    const prompt = expressionFitPrompt({
      ...
      tradeExpression: {
        ...
        reason:
          "Verbose stage summary that should not be sent to fit assessment.",
        directAssetTradable: true,
        marketDiscoveryConfidence: 0.4,
        publicMarketReadThrough: "moderate",
        insufficiency: {
