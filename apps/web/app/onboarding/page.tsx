...
function StyledQR({ data, size = 196 }: { data: string; size?: number }) {
      ...
      {finders.map((f, i) => {
        ...
        return (
          <g key={`finder-${i}`}>
            ...
            <rect x={fx + cell} y={fy + cell} width={fs - 2 * cell} height={fs - 2 * cell} rx={(fs - 2 * cell) * 0.24} fill="#ffffff" />
            <rect x={fx + 2 * cell} y={fy + 2 * cell} width={fs - 4 * cell} height={fs - 4 * cell} rx={(fs - 4 * cell) * 0.26} fill="#0a0a0a" />
          </g>
        );
      })}
      {dots}
      <rect x={logoX - pad} y={logoY - pad} width={logoW + pad * 2} height={logoW + pad * 2} rx={cell * 0.6} fill="#ffffff" />
