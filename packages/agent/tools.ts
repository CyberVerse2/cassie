...
export function createCassieSupervisorTools(input: {
  ...
  return {
    preflight_user_policy: tool({
      description:
        "Run deterministic user-policy preflight before semantic opportunity analysis.",
      inputSchema: z.object({}),
      execute: async () =>
        runStepOnce("preflight", {}, async () =>
          recordRunStep({
            store: input.store,
    ...
    classify_source_mode: tool({
      description:
        "Classify whether the source content is normal or breaking news while preserving user intent.",
      inputSchema: z.object({}),
      execute: async () =>
        runStepOnce("intake", { kind: "source_mode" }, async () => {
    ...
    frame_opportunity: tool({
      description:
        "Frame the market opportunity implied by the source post without choosing the final trade.",
      inputSchema: z.object({}),
      execute: async () =>
        runStepOnce("opportunity", {}, async () => {
          const source = await sourceForAnalysis();
          return recordRunStep({
            store: input.store,
            runId: input.run.runId,
    ...
    resolve_source: tool({
      description:
        "Resolve a real X/Twitter status URL into Cassie's SourcePost text shape",
      inputSchema: z.object({
        url: z.string().url(),
      }),
      execute: async ({ url }) =>
        runStepOnce("intake", { url }, async () => {
          if (!input.deps.sourceResolver) {
            throw new Error(
              "resolve_source requires a configured source resolver dependency.",
            );
          }
          const output = await recordRunStep({
            ...
            stepInput: { url },
            execute: ({ setThinkingTrace }) =>
              input.deps.sourceResolver!.resolveSource({
                url,
    ...
    generate_trade_expressions: tool({
      description:
        "Generate candidate trade expressions in one AI step; do not search venues or create a ticket.",
      inputSchema: z.object({
        opportunityFrame: OpportunityFrameSchema.optional(),
        marketCandidates: MarketCandidateSchema.array().optional(),
      }),
      execute: async ({ opportunityFrame, marketCandidates }) =>
        runStepOnce(
          "trade_expression",
    ...
    search_venues: tool({
      description:
        "Search configured execution and market venues for real candidates matching the trade expression.",
      inputSchema: z.object({
        venues: z.array(z.enum(["hyperliquid", "polymarket"])).optional(),
        limit: z.number().int().positive().max(25).optional(),
      }),
      execute: async ({ venues, limit }) => {
        const tradeExpression = await requireLatestTradeExpression(
          input.store,
          input.run.runId,
        );
        return runStepOnce("market_candidates", { venues, limit }, async () => {
    ...
    assess_expression_fit: tool({
      description:
        "Use AI semantic judgment to assess whether a real venue candidate fits the framed opportunity and intended expression.",
      inputSchema: z.object({
        candidate: MarketCandidateSchema,
        side: z.enum(["yes", "no"]).optional(),
      }),
      execute: async ({ candidate, side }) => {
        const opportunityFrame = await requireLatestOpportunityFrame(
          input.store,
          input.run.runId,
        );
        const tradeExpression = await requireLatestTradeExpression(
        ...
        const candidateSide = predictionMarketSideForCandidate(
          persistedCandidate,
          side,
        );
        return runStepOnce(
          "market_assessment",
          {
            ...
            ...(candidateSide ? { side: candidateSide } : {}),
          },
          async () => {
            return recordRunStep({
              store: input.store,
    ...
    quote_expression: tool({
      description:
        "Refresh quote data for a validated candidate. Do not quote rejected or unassessed expressions.",
      inputSchema: z.object({
        candidate: MarketCandidateSchema,
      ...
      execute: async ({ candidate, fitAssessment, side }) => {
        ...
        return runStepOnce(
          ...
          {
            candidate: persistedCandidate,
            fitAssessment: persistedFitAssessment,
            ...(candidateSide ? { side: candidateSide } : {}),
          },
          async () => {
            if (persistedFitAssessment.fitStatus !== "validated") {
              throw new Error(
                "quote_expression requires a validated fit assessment.",
              );
            }
            return recordRunStep({
              store: input.store,
    ...
    rank_expressions: tool({
      description:
        "Rank real venue candidates and choose the best grounded trade expression; do not invent markets.",
      inputSchema: z.object({
      ...
      execute: async ({ candidates, fitAssessments, quotes }) => {
        ...
        const storedCandidates = await latestPersistedMarketCandidates(
          input.store,
          input.run.runId,
        );
        ...
        const storedFitAssessments = await latestPersistedFitAssessments(
          input.store,
          input.run.runId,
        );
        const persistedFitAssessments =
          storedFitAssessments.length > 0
            ? storedFitAssessments
            : parseSuppliedFitAssessments(fitAssessments);
        const persistedQuotes = await latestPersistedQuotes(
          input.store,
          input.run.runId,
        );
        const groundedQuotes =
          persistedQuotes.length > 0
            ? persistedQuotes
            : parseSuppliedRankQuotes(quotes);
        const validatedFitAssessments = persistedFitAssessments.filter(
          (assessment) => assessment.fitStatus === "validated",
        );
        const selectedFitAssessment = bestFitAssessment(
          validatedFitAssessments,
        );
        const selectedCandidate = selectedFitAssessment
          ? candidateForFitAssessment(
              persistedCandidates,
        ...
        if (groundedQuotes.length === 0) {
          throw new Error(
            "rank_expressions requires a persisted or supplied market quote.",
          );
        }
        if (!selectedFitAssessment || !selectedCandidate) {
          throw new Error(
            "rank_expressions requires at least one validated venue candidate.",
          );
        }
        if (
          !groundedQuotes.some((quote) =>
            quoteMatchesCandidate(quote, selectedCandidate),
          )
        ) {
          throw new Error(
            "rank_expressions requires a quote for the best validated venue candidate.",
          );
        }
    ...
    create_trade_ticket: tool({
      description:
        "Create a trade ticket from the selected market using the user's configured default trade size and an explicit exit plan. This never executes the order directly.",
      inputSchema: z.object({
      ...
      execute: async ({ tradeExpression, marketSelection, exitPlan }) => {
        const persistedTradeExpression =
          (await latestPersistedTradeExpression(
            input.store,
            input.run.runId,
          )) ?? parseSuppliedTradeExpression(tradeExpression);
        const persistedMarketSelection =
          (await latestPersistedMarketSelection(
            input.store,
            input.run.runId,
        ...
        return runStepOnce(
          ...
          async () => {
            ...
            const thesis = thesisForMarketSelection(
              persistedTradeExpression,
              persistedMarketSelection,
            );
            return recordRunStep({
              store: input.store,
              runId: input.run.runId,
    ...
    finalize_run: tool({
      description:
        "Finalize the Cassie run with the user-facing result after analysis, no-trade, or trade-ticket creation.",
      inputSchema: FinalizeRunInputSchema,
      execute: async (finalInput) =>
        runStepOnce("final", finalInput, async () => {
...
async function resolveLatestMarketCandidate(input: {
  ...
  const candidates = await latestPersistedMarketCandidates(
    input.store,
    input.runId,
  );
  ...
  const matches = candidates.filter(
    (candidate) => marketCandidateLookupKey(candidate) === requestedKey,
  );
  if (matches.length === 1) return matches[0]!;
  if (matches.length > 1) {
    throw new Error(
      `Candidate ${requestedKey} is ambiguous across latest venue-search results.`,
    );
  }
  throw new Error(
    `Candidate ${requestedKey} was not found in persisted venue-search results.`,
  );
}

async function latestPersistedMarketCandidates(
  store: CassieStore,
  runId: string,
): Promise<MarketCandidate[]> {
  const latestStep = await latestSucceededStep(
    store,
    runId,
    "market_candidates",
  );
...
async function latestPersistedFitAssessments(
  store: CassieStore,
  runId: string,
...
async function latestPersistedMarketSelection(
  ...
  return latestStep ? MarketSelectionSchema.parse(latestStep.output) : null;
}

async function latestPersistedOpportunityFrame(
  store: CassieStore,
  runId: string,
): Promise<OpportunityFrame | null> {
  const latestStep = await latestSucceededStep(store, runId, "opportunity");
  return latestStep ? OpportunityFrameSchema.parse(latestStep.output) : null;
}

async function requireLatestOpportunityFrame(
  store: CassieStore,
  runId: string,
  ...
  if (!opportunityFrame) {
    throw new Error(
      "assess_expression_fit requires a persisted opportunity frame from frame_opportunity.",
    );
  }
  return opportunityFrame;
}

async function latestPersistedTradeExpression(
  store: CassieStore,
  runId: string,
) {
  const latestStep = await latestSucceededStep(
  ...
  return latestStep ? TradeExpressionPlanSchema.parse(latestStep.output) : null;
}

async function requireLatestTradeExpression(
  store: CassieStore,
  runId: string,
  ...
  if (!tradeExpression) {
    throw new Error(
      "This tool requires a persisted trade expression from generate_trade_expressions.",
    );
  }
  return tradeExpression;
}

async function latestPersistedQuotes(
  store: CassieStore,
  runId: string,
): Promise<unknown[]> {
  const steps = await store.getRunSteps(runId);
  return steps
    .filter(
      (step) => step.stepType === "market_quote" && step.status === "succeeded",
    )
    .sort((left, right) => left.startedAt.localeCompare(right.startedAt))
...
function parseSuppliedMarketCandidates(value: unknown): MarketCandidate[] {
  ...
  throw new Error(
    "rank_expressions requires real venue candidates from search_venues.",
  );
}

function parseSuppliedFitAssessments(
  value: unknown,
...
function parseSuppliedRankQuotes(value: unknown): unknown[] {
  const parsed = z
    .array(z.union([MarketCandidateSchema, PolymarketQuoteSchema]))
    .safeParse(value);
  if (parsed.success) return parsed.data;
...
function candidateForFitAssessment(
  ...
  if (!match) {
    throw new Error(
      `Validated candidate ${fitAssessment.candidateId} was not found in persisted venue-search results.`,
    );
  }
  return match;
}

function quoteMatchesCandidate(
  quote: unknown,
  candidate: MarketCandidate,
): boolean {
  ...
  if (quotedCandidate.success) {
    return (
      marketCandidateLookupKey(quotedCandidate.data) ===
...
function parseSuppliedTradeExpression(value: unknown) {
  ...
  if (parsed.success) return parsed.data;
  throw new Error(
    "Trade ticket creation requires a persisted or supplied trade expression.",
  );
}
...
async function latestSucceededStep(
  ...
  return (
    steps
      .filter(
        (step) => step.stepType === stepType && step.status === "succeeded",
      )
      .sort((left, right) =>
...
async function resolveLatestFitAssessment(input: {
  ...
  const assessments = await latestPersistedFitAssessments(
    input.store,
    input.runId,
  );
  const match = assessments.find(
    (assessment) =>
      normalizedCandidateId(assessment.candidateId) ===
        normalizedCandidateId(input.fitAssessment.candidateId) &&
  ...
  throw new Error(
    `Fit assessment ${input.fitAssessment.candidateId} was not found in persisted expression-fit results.`,
  );
}
