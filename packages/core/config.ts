...
const configuredStringSchema = z.preprocess(
...
export const RuntimeConfigSchema = z
  ...
  .superRefine((values, context) => {
    if (
      !values.DATABASE_URL &&
      (!values.CASSIE_POSTGRES_HOST || !values.CASSIE_POSTGRES_PASSWORD)
    ) {
      context.addIssue({
        code: "custom",
        message:
          "DATABASE_URL or CASSIE_POSTGRES_HOST/CASSIE_POSTGRES_PASSWORD is required.",
        path: ["DATABASE_URL"],
      });
    }
  });
...
export function readCassieConfig(
  ...
  return {
    ...
    structuredAi: {
      maxRetries: numberEnv("CASSIE_STRUCTURED_MAX_RETRIES", 2, env, {
        integer: true,
        min: 1,
      }),
    },
    ...
    x: xWebhookEnv(env),
    supervisor: {
      mode: supervisorModeEnv(env),
      timeoutMs: numberEnv("CASSIE_SUPERVISOR_TIMEOUT_MS", 1_800_000, env, {
        integer: true,
        min: 1,
      }),
      stepTimeoutMs: numberEnv(
        "CASSIE_SUPERVISOR_STEP_TIMEOUT_MS",
        900_000,
        env,
        { integer: true, min: 1 },
      ),
...
function supervisorModeEnv(env: EnvSource): "pipeline" | "tool_loop" {
  const mode = optionalEnv("CASSIE_SUPERVISOR_MODE", env);
  if (!mode) return "pipeline";
  if (mode === "pipeline" || mode === "tool_loop") return mode;
  throw new Error("CASSIE_SUPERVISOR_MODE must be pipeline or tool_loop.");
}
...
export function readDatabaseUrl(
  ...
  const password = requiredConfiguredStringSchema().parse(
    env.CASSIE_POSTGRES_PASSWORD,
  );
  const user = optionalEnv("CASSIE_POSTGRES_USER", env) ?? "postgres";
  const database = optionalEnv("CASSIE_POSTGRES_DATABASE", env) ?? "cassie";
  const port = numberEnv("CASSIE_POSTGRES_PORT", 5432, env, {
    integer: true,
    min: 1,
    max: 65535,
  });
...
export function readDatabasePoolEnv(env: EnvSource = process.env) {
  return z
    .object({
      CASSIE_DATABASE_POOL_MAX: numberSchema("CASSIE_DATABASE_POOL_MAX", 10, {
        integer: true,
        min: 1,
      }),
      CASSIE_DATABASE_CONNECTION_TIMEOUT_MS: numberSchema(
        "CASSIE_DATABASE_CONNECTION_TIMEOUT_MS",
        5_000,
        {
          integer: true,
          min: 1,
      ...
      CASSIE_DATABASE_IDLE_TIMEOUT_MS: numberSchema(
        "CASSIE_DATABASE_IDLE_TIMEOUT_MS",
        30_000,
        {
          integer: true,
          min: 1,
      ...
      CASSIE_DATABASE_MAX_LIFETIME_SECONDS: numberSchema(
        "CASSIE_DATABASE_MAX_LIFETIME_SECONDS",
        300,
        {
          integer: true,
          min: 1,
...
export function readAiProviderEnv(
  ...
  return aiProviderEnvSchema
    .transform((values) => ({
      googleApiKey: firstConfigured(
      ...
      cheapModel:
        firstConfigured(values.CASSIE_CHEAP_MODEL, values.DEEPSEEK_MODEL) ??
        defaults.cheapModel,
      importantModel:
        firstConfigured(
          values.CASSIE_IMPORTANT_MODEL,
          values.CASSIE_EXPENSIVE_MODEL,
          values.CASSIE_MODEL,
        ) ?? defaults.importantModel,
      webSearchModel:
        firstConfigured(
          values.CASSIE_WEB_SEARCH_MODEL,
          values.GEMINI_WEB_SEARCH_MODEL,
        ) ?? defaults.webSearchModel,
      grokXSearchModel:
        values.GROK_X_SEARCH_MODEL ?? defaults.grokXSearchModel ?? "grok-4.3",
    }))
    .parse(env);
}

export function readGraphileWorkerEnv(
  env: EnvSource = process.env,
): GraphileWorkerEnv {
  return z
    .object({
      GRAPHILE_EXECUTION_MAX_ATTEMPTS: numberSchema(
        "GRAPHILE_EXECUTION_MAX_ATTEMPTS",
        5,
        { integer: true, min: 1 },
      ),
      GRAPHILE_SUPERVISOR_MAX_ATTEMPTS: numberSchema(
        "GRAPHILE_SUPERVISOR_MAX_ATTEMPTS",
        3,
        { integer: true, min: 1 },
      ),
      GRAPHILE_WORKER_CONCURRENCY: numberSchema(
        "GRAPHILE_WORKER_CONCURRENCY",
        1,
        { integer: true, min: 1 },
      ),
      GRAPHILE_WORKER_POLL_INTERVAL_MS: numberSchema(
        "GRAPHILE_WORKER_POLL_INTERVAL_MS",
        2_000,
        { integer: true, min: 1 },
      ),
      CASSIE_POSITION_REVIEW_INTERVAL_MINUTES: numberSchema(
        "CASSIE_POSITION_REVIEW_INTERVAL_MINUTES",
        1,
        { integer: true, min: 1, max: 59 },
      ),
      CASSIE_X_MENTION_POLL_INTERVAL_MINUTES: numberSchema(
        "CASSIE_X_MENTION_POLL_INTERVAL_MINUTES",
        1,
        { integer: true, min: 1, max: 59 },
      ),
...
export function readHttpRuntimeEnv(
  env: EnvSource = process.env,
): HttpRuntimeEnv {
  return z
    .object({
      PORT: numberSchema("PORT", 3000, { integer: true, min: 1 }),
      CASSIE_RATE_LIMIT_MAX: numberSchema("CASSIE_RATE_LIMIT_MAX", 60, {
        integer: true,
        min: 1,
      }),
      CASSIE_RATE_LIMIT_WINDOW_MS: numberSchema(
        "CASSIE_RATE_LIMIT_WINDOW_MS",
        60_000,
        { integer: true, min: 1 },
      ),
    })
...
export function readPrivyEnv(env: EnvSource = process.env): PrivyEnv {
  return z
    .object({
      ...
      PRIVY_WALLET_ACTION_POLL_INTERVAL_MS: numberSchema(
        "PRIVY_WALLET_ACTION_POLL_INTERVAL_MS",
        1_500,
        {
          integer: true,
          min: 100,
      ...
      PRIVY_WALLET_ACTION_POLL_TIMEOUT_MS: numberSchema(
        "PRIVY_WALLET_ACTION_POLL_TIMEOUT_MS",
        120_000,
        {
          integer: true,
          min: 1_000,
    ...
    .transform((values) => ({
      appId: firstConfigured(
        values.PRIVY_APP_ID,
        values.NEXT_PUBLIC_PRIVY_APP_ID,
      ),
...
export function assertPrivySettlementEnv(
  config: PrivyEnv,
): RequiredPrivySettlementEnv {
...
export function assertAuthEnv(config: AuthEnv): RequiredAuthEnv {
  const missing = [
    config.secret ? null : "BETTER_AUTH_API_KEY",
    config.twitterClientId ? null : "BETTER_AUTH_X_CLIENT_ID",
    config.twitterClientSecret ? null : "BETTER_AUTH_X_CLIENT_SECRET",
  ].filter((name): name is string => Boolean(name));
  if (missing.length > 0) {
...
export function readTelegramEnv(env: EnvSource = process.env): TelegramEnv {
  return z
    .object({
      ...
      TELEGRAM_CONNECT_TTL_MS: numberSchema(
...
export function assertTelegramBotEnv(
  config: TelegramEnv,
): RequiredTelegramBotEnv {
  if (!config.botToken) {
...
export function assertTelegramConnectEnv(
  config: TelegramEnv,
): RequiredTelegramConnectEnv {
...
export function readHyperliquidExecutionEnv(
  ...
  const schema = z
    .object({
      ...
      HYPERLIQUID_EXECUTION_SLIPPAGE_BPS: numberSchema(
        "HYPERLIQUID_EXECUTION_SLIPPAGE_BPS",
        100,
        { min: 0 },
      ),
      HYPERLIQUID_PRICE_DECIMALS: numberSchema(
        "HYPERLIQUID_PRICE_DECIMALS",
        5,
        { integer: true, min: 0 },
      ),
      HYPERLIQUID_PERP_LEVERAGE: numberSchema(
        "HYPERLIQUID_PERP_LEVERAGE",
        DEFAULT_HYPERLIQUID_PERP_LEVERAGE,
        { integer: true, min: 1 },
      ),
    })
    .transform((values) => {
      const privateKey = firstConfigured(
        options.privateKey,
        values.HYPERLIQUID_PRIVATE_KEY,
      );
      const mainPrivateKey = firstConfigured(
        options.mainPrivateKey,
        values.HYPERLIQUID_MAIN_PRIVATE_KEY,
      );

      return {
        privateKey: privateKey
          ? normalizePrivateKey(privateKey, "HYPERLIQUID_PRIVATE_KEY")
          : undefined,
        mainPrivateKey: mainPrivateKey
          ? normalizePrivateKey(mainPrivateKey, "HYPERLIQUID_MAIN_PRIVATE_KEY")
          : undefined,
        slippageBps:
          options.slippageBps ?? values.HYPERLIQUID_EXECUTION_SLIPPAGE_BPS,
        priceDecimals:
          options.priceDecimals ?? values.HYPERLIQUID_PRICE_DECIMALS,
  ...
  const schema = z
    ...
    .transform((values) => {
      ...
      const apiKey = firstConfigured(
        options.apiKey,
        values.POLYMARKET_CLOB_API_KEY,
      );
      const apiSecret = firstConfigured(
        options.apiSecret,
        values.POLYMARKET_CLOB_SECRET,
      );
      const apiPassphrase = firstConfigured(
        options.apiPassphrase,
        values.POLYMARKET_CLOB_PASS_PHRASE,
      );
      ...
      return {
        ...
        creds:
          apiKey && apiSecret && apiPassphrase
            ? {
                key: apiKey as ApiKeyCreds["key"],
                secret: apiSecret,
                passphrase: apiPassphrase,
              }
            : undefined,
        host:
          firstConfigured(options.host, values.POLYMARKET_CLOB_HOST) ??
          "https://clob.polymarket.com",
        rpcUrl:
          firstConfigured(options.rpcUrl, values.POLYMARKET_RPC_URL) ??
          "https://polygon-rpc.com",
        signatureType:
          options.signatureType ??
          parsePolymarketSignatureType(values.POLYMARKET_SIGNATURE_TYPE),
        funderAddress: firstConfigured(
          options.funderAddress,
          values.POLYMARKET_FUNDER_ADDRESS,
        ),
        builderCode: firstConfigured(
          options.builderCode,
          values.POLYMARKET_BUILDER_CODE,
        ),
        relayerApiKey: firstConfigured(
          options.relayerApiKey,
          values.POLYMARKET_RELAYER_API_KEY,
        ),
        relayerApiKeyAddress: firstConfigured(
          options.relayerApiKeyAddress,
          values.POLYMARKET_RELAYER_API_KEY_ADDRESS,
        ),
      };
    });
...
function aiProviderEnvDefaults(
  overrides: Partial<AiProviderEnvDefaults> = {},
): AiProviderEnvDefaults {
  return {
